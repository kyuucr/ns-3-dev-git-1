...
bool
RequestQueue::Enqueue (QueueEntry & entry, uint32_t node_id)
{
  Purge ();
  ...
  if (m_queue.size () == m_maxLen)
    {
      //std::cout<<"Estoy dropeando un paquete por exceso en cola ("<<m_maxLen<<") en el nodo: "<<node_id<<" en tiempo: "<<Simulator::Now()<<std::endl;
